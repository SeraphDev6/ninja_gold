...
def process_money():
    ...
    if request.form['building'] == 'farm':
        ...
        session['events'].insert(0,f"<li class='text-success'>{now}: Earned {earned} gold From the Farm!</li>")
    elif request.form['building'] == 'cave':
        earned=randint(5,10)
        session['gold'] = str(int(session['gold'])+ earned)
        session['events'].insert(0,f"<li class='text-success'>{now}: Earned {earned} gold From the Cave!</li>")
    elif request.form['building'] == 'house':
        earned=randint(2,5)
        session['gold'] = str(int(session['gold'])+ earned)
        session['events'].insert(0,f"<li class='text-success'>{now}: Earned {earned} gold From the House!</li>")
